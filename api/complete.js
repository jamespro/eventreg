...
//TODO: I'm not using axios, so I think I need to modify savedComplete to save the record to the mongodb database instead of this syntax; instead of save(). Check controllers.
    const savedComplete = await newComplete.save().catch((err) => {
    console.log("Error: ", err);
    res.status(500).json({ error: "Cannot register Complete at the moment!" });
  });

  if (savedComplete) res.json({ message: "Your order has been submitted" });
});
